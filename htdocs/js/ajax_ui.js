...
function get_music_info(url, query_string=null, header=null)//请求url,请求参数(对象),请求头(对象)
{
	...
	http_request.onreadystatechange=function(){
		if(http_request.readyState === XMLHttpRequest.DONE && http_request.status === 200){
			//正确返回
			music_info.value = JSON.parse(http_request.responseText);
			music_info.status = 1;
		}
	};
}
...
function get_music_page_list(url, query_string=null, header=null)//请求url,请求参数(对象),请求头(对象)
{
	var http_request = null;
	if (window.XMLHttpRequest){//chrome firefox...
		http_request = new XMLHttpRequest();
	...
	http_request.onreadystatechange=function(){
		if(http_request.readyState === XMLHttpRequest.DONE && http_request.status === 200){
			...
			page_list.value = JSON.parse(http_request.responseText);
			page_list.num = page_list.value.length;
			page_list.status = 1;
		}
	};
}